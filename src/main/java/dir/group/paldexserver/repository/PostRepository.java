...
public interface PostRepository extends JpaRepository<PostEntity, Long> {
    @Query(value = "SELECT p.pk AS pk, p.title AS title, " +
            ...
            " p.is_public AS isPublic, p.is_review AS isReview, p.comment AS comment, f.path AS path FROM post p INNER JOIN file f ON p.pk = f.tpk AND f.t_name = 'post' AND f.is_thumb = '1'", nativeQuery = true)
    ...
    @Query(value = "SELECT p.pk AS pk, p.title AS title, " +
            ...
            "isReview, p.comment AS comment, f.path AS path FROM post p LEFT JOIN file f ON p.pk = f.tpk AND f.t_name = 'post' AND f.is_thumb = '1' WHERE FIND_IN_SET(:tag, REPLACE(p.tags, ', ', ',')) > 0", nativeQuery = true)
